...
TEST_CASE("process monitor respawns up to limit") {
    ...
    opts.service.respawn_max = 2;
    opts.service.respawn_window = std::chrono::minutes(1);
    run_with_monitor(opts);
...
TEST_CASE("process monitor uses custom respawn delay") {
    ...
    opts.service.respawn_max = 2;
    opts.service.respawn_window = std::chrono::minutes(1);
    opts.service.respawn_delay = std::chrono::milliseconds(50);
    auto start = std::chrono::steady_clock::now();
    run_with_monitor(opts);
    auto elapsed = std::chrono::steady_clock::now() - start;
    REQUIRE(g_monitor_count == 2);
...
TEST_CASE("process monitor exponential backoff on failures") {
    ...
    Options opts;
    opts.service.persist = true;
    opts.service.respawn_max = 3;
    opts.service.respawn_window = std::chrono::minutes(1);
    opts.service.respawn_delay = std::chrono::milliseconds(10);
    auto start = std::chrono::steady_clock::now();
    run_with_monitor(opts);
    auto elapsed = std::chrono::steady_clock::now() - start;
