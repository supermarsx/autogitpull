@echo off setlocal set "SCRIPT_DIR=%~dp0"
        ...
        nul if errorlevel 1(echo clang++ not found.Attempting to install LLVM via
            ...
            rem ────────────────────────────────────────────────────────────── rem
        2. libgit2 paths set "LIBGIT2_INC=libs\libgit2\_install\include" set
    ...
    AddressSanitizer
                ...
                rem Common flags set "CXX=clang++" set
                                     "CXXFLAGS=-std=c++20 -O0 -g -fsanitize=address" set
            ...
            Iinclude
        ^
        src\autogitpull.cpp src\git_utils.cpp src\tui.cpp src\logger.cpp src\resource_utils
            .cpp src\system_utils.cpp src\time_utils.cpp src\debug_utils.cpp src\parse_utils.cpp
        ^ src\config_utils.cpp src\options.cpp src\lockfile.cpp ^ "%LIBGIT2_LIB%\libgit2.a" ^
